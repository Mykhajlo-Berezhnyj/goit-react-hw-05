...
export default function MovieDetailsPage() {
    ...
    return (
        <div>
            <button className={css['btn-back']} type='button' onClick={handleGoBack}>Back</button>
            {loading && <Loader />}
            {error && <ErrorMessage />}
            {data &&
                (<div className={css.container}>
                    {`${data.poster_path}` && <img src={`https://image.tmdb.org/t/p/w200${data.poster_path}`} alt={data.title || data.name} />}
