...
export default function MovieItem({ movie }) {
    ...
    return (
        <Link to={`/movies/${movie.id}`} onClick={handleClick} aria-label={`Open details for ${movie.title || movie.original_title || movie.name}`}>
                <div className={`${css.card} ${isDetailsPage ? css.border : ""}`}>
                    ...
                    <div className={css.overview}>
                        <h2 className={css['title-movie']}>{movie.title|| movie.original_title || movie.name}</h2>
                        <p>{movie.overview}</p>
